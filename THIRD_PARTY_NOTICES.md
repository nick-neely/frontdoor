...
- The three employer marks in the timeline on `/work` are not tool logos and did not come from svgl. Each one identifies the employer whose Role it sits beside and implies no affiliation with or endorsement by its owner beyond the employment that row describes.

  - Lean TECHniques' compact LT mark is drawn inline in `src/components/lean-techniques-mark.tsx`, traced from the company's official wordmark and painted in `currentColor` so it follows the surrounding text in both themes. It also appears in the home page's Now Line. It remains the trademark of Lean TECHniques.

  - `neely-solutions.svg` and `neely-solutions-dark.svg` sit here for locality rather than because they are third-party: Neely Solutions is Nick's own brand, and the two files are the marks that brand's own site serves from its `public/logo-dark.svg` and `public/logo-light.svg`. Those originals are named for their ink; the vendored copies carry this repository's naming, which is named for the ground, so the pair swaps the same way every other pair in `public/logos/` does. The artwork is unmodified. The only edit is the `<title>` and `aria-label`, which in the originals still carried the filename of the raster the artwork was traced from.

  - `moore-hearing-clinic.webp` is Moore Hearing Clinic's mark, used with the owner's permission by way of Nick, and processed here from the 1080x1080 PNG the clinic supplied. Two changes, neither to the artwork: it is masked to a circle at the outer edge of its teal ring, so the square plate the clinic's file paints around it drops out and the mark carries its own contrast on both of this site's grounds, and it is resampled to 72px, four times the 18px it renders at. It remains the clinic's mark.
