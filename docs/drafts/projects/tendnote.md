...
- **Person detail page — not captured. Blocked, deliberately.** This is the richest surface and the most obvious missing shot. I stopped because the local database is _not_ clean: the People directory contains a row for **"Juli" — Partner, Birthday February 10**. The repo's own publication audit (`docs/phase-9a/publication-inventory.md`) identifies Juli as a real person whose name was replaced with the fixture "Mara" throughout the tree. `git grep` confirms it: the fixture names in the shots I kept (Jordan Rivera, Priya Shah, Casey Thompson, Sam Patel, Alex Morgan, Dana Kim) each appear in 5–15 tracked repo fixture files, while "Juli" appears in exactly one — the audit doc that flags her as real. I deleted the People-directory capture I had already taken and did not photograph any person detail page. **Re-seed a clean database and these are a five-minute recapture.**
...
5. **Re-seed for screenshots.** The local dev database mixes fixtures with at least one real person (Juli). I skipped every person-level surface because of it. If you reset to `pnpm db:seed` fixtures — or confirm which people are safe — the person detail page, Eve chat, and Global Recall shots are quick to capture.
