...
### Held in reserve - cut from the page, kept here

Nick's call (2026-08-25): four decisions, in the order above. These two are the ones to drop first. They are kept verbatim because their Facts register rows still point at them, and because the version pin is the right thing to reach for if the page ever needs a fifth.
...
- **Latest release:** `v0.1.0-preview.5`, published 2026-05-22, macOS / Windows / Linux artifacts with published SHA-256 checksums. **This is the documented milestone date for pilog** (Nick, 2026-08-25) - the one to use if the home Update feed wants a date.

**Status, settled (Nick, 2026-08-25).** Pilog's status is Shipped and stays Shipped. It is technically shipped: the app is built, released, and installable on macOS, Windows and Linux. The reason every published artifact is a prerelease is code signing and nothing else - Nick chose not to pay for a signing certificate, so builds are signing-ready but unsigned and installers show an OS security warning. The release manifest's `stable: null` and the download page's "No stable release yet" are that single decision showing through, not an unfinished product. The page can and should say exactly that: preview builds are the current distribution because signing is a cost decision, not because the work isn't done.
...
| 62 | Status is Shipped; unsigned prereleases are a signing-cost decision, not an unfinished release | Nick, direct answer, 2026-08-25 |
| 63 | 2026-05-22 (the `v0.1.0-preview.5` release) is the documented milestone date | Nick, direct answer, 2026-08-25, agreeing with row 59 |
| 64 | The `v1.0.0` tag exists only in the local clone and was never pushed; `git ls-remote` shows only the five preview tags | Nick, direct answer, 2026-08-25, with `git ls-remote` cited by him |
| 65 | Pi may be named outright: pilog is built on Pi, the minimal coding harness, under the hood | Nick, direct answer, 2026-08-25 |
| 66 | Pilog runs completely locally when configured with Ollama and local models, or sends work to cloud providers for better processing; it is a configuration choice | Nick, direct answer, 2026-08-25 (widens row 17, which is the site's own wording) |
...
1. **Status and the stable-release gap.** Status stays Shipped. Pilog is technically shipped; signing is the only reason every release is a prerelease, because Nick chose not to pay for a signing certificate. Section 6 now states that plainly instead of flagging a tension.
2. **The milestone date.** `2026-05-22`, the `v0.1.0-preview.5` release, is the documented date. Recorded in section 6 and Facts register row 63.
3. **The stray `v1.0.0` tag.** It exists only in the local clone and was never on GitHub - `git ls-remote` shows only the five preview tags. Nick pushed something like it accidentally once and rectified it. Nothing is visible to visitors and no action is needed.
4. **How much of Pi to name.** Name it. Pilog is built on Pi, the minimal coding harness, under the hood; section 3 now says so outright instead of hedging toward "an embedded open-source agent runtime".
5. **Which decisions to keep.** Nick agrees with the recommendation. Section 4 now runs four, in the recommended order: read-only by construction, in-process embedding, the prompt quality loop, then Device Flow / `safeStorage`. The version pin and the release manifest moved to "Held in reserve" inside section 4 rather than being deleted, so their Facts register rows still resolve.
6. **The privacy sentence.** Keeps its prominence at the end of section 2, and gains the missing half of the picture: pilog runs completely locally against Ollama and local models, or sends work to a cloud provider for better processing, depending on how you configure it. Both are first-class; the sentence no longer reads as a one-way concession.
