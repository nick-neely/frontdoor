...
**Source repo:** `/home/neely/dev/diffbill` @ `6fed8995` (`origin/main`, the merge of PR #163). Nothing in that repo was modified.
...
>
> A second round - diffbill PR #163 ("fix(core): close source redaction gaps", merged 2026-08-25 as `6fed8995`) - closed three residual issues found while verifying #159. Each was confirmed against the merged code on 2026-08-26:
>
> - **#160** - the translate-work error fallback no longer passes client-supplied source text through untouched. It redacts server-side, under the same fail-closed entitlement gate the sources route uses, and the same redaction now runs before invoice sources are persisted. The resolution chosen was to redact, not to refuse the fallback.
> - **#161** - the diff-excerpt trimmer redacts the whole patch before truncating it, so a secret straddling character 400 is redacted rather than half-published. A boundary regression test covers it.
> - **#162** - **partial.** Three shapes were added: PEM-armored private keys, Stripe live and webhook secrets, and credential-bearing database connection strings. JWTs and AWS secret access keys were suggested on the issue and did not land. Section 3 is written to the shapes the code actually matches.
>
...
_The ordering and the trust boundary were **verified fixed in diffbill PR #163 (merged 2026-08-25), confirmed against code 2026-08-26** - diffbill **#161** and **#160**. On ordering: `translation-context.ts:215-219` (`trimPatch`) redacts the full patch and truncates the redacted text, so the 400-character cut can no longer split a secret into a prefix the regex misses; `translation-context.test.ts` places a secret across character 400 and asserts a 400-character excerpt ending in `[REDACTED]`. On the trust boundary: the fallback paths no longer return the caller's own text unexamined - `translation-context.ts:47` and `:52` redact the client-supplied source before returning it when the repo name will not parse or GitHub enrichment throws, `app/api/translate-work/route.ts:449-451` redacts the caller's sources in the route's outer catch, and `app/api/invoices/route.ts:164-166` redacts before sources are persisted. All of it runs under `redactSensitive`, resolved fail-closed as `entitlements.redactionControl ? requested : true` at `translate-work/route.ts:430-432` - the same gate `app/api/github/sources/route.ts:69-71` and `lib/services/clients.ts:501-505` apply, and `lib/defaults.ts:12` defaults the setting on. **The resolution chosen was server-side redaction, not refusal of the fallback.** Clear to publish._

_Why section 3 names the shapes it names: **diffbill #162 landed partially** (PR #163, confirmed against code 2026-08-26). `apps/core-app/lib/validators/sanitize.ts:1-10` now carries eight patterns - PEM private-key blocks (L2), `gh[pousr]_` tokens (L3), `github_pat_` (L4), Stripe `sk_live_` / `rk_live_` / `whsec_` (L5), `postgres` / `postgresql` / `mysql` / `mariadb` / `mongodb` / `mongodb+srv` / `redis` / `rediss` URLs carrying an inline `user:password@` (L6), 32-or-more hex characters (L7), email addresses (L8), and 20-or-more-character screaming-snake constants (L9). Three of those are new. **JWTs are not covered, and neither are AWS secret access keys** - both were suggested on #162 and neither pattern landed. PR #163's description credits "AWS access key IDs"; there is no AWS pattern in the file, and the `AKIAIOSFODNN7EXAMPLE` fixture in `sanitize.test.ts` passes only because an AWS key ID happens to be twenty uppercase alphanumerics and is caught by the pre-existing screaming-snake rule at L9 - which an AWS *secret* key's mixed case and `/` characters escape. Also uncovered: `sk_test_` and `rk_test_` Stripe keys, Slack `xoxb-` tokens, OpenAI and Google API keys, unarmored key material, and connection-string schemes outside that list such as `amqp://` or an HTTPS basic-auth URL. Section 3's sentence is written to that reality and must not be edited into implying entropy detection._

_**Residual caveat, carried forward: the body excerpt is still truncated before it is redacted.** #161 was scoped to `trimPatch`, and `trimPatch` is fixed - but the same failure survives on a different field. `translation-context.ts:74` builds `bodyExcerpt` with `createExcerpt(detail.body)`, which normalizes whitespace and slices to 240 characters, and `redactWorkSourceExcerpts` only runs afterwards at `:88`; `lib/github/sources.ts:238` does the same and redacts at `:159-160`. A `ghp_` token beginning near character 235 of a PR body loses the tail that makes it match `{20,}` and its prefix survives into the stored excerpt. Confirmed by reproducing the regex against a truncated fixture on 2026-08-26. Section 3 scopes its before-truncation claim to the diff excerpts, so nothing on this page overstates it - but this wants its own diffbill issue._
...
| 79 test files | `find apps packages -name "*.test.ts*"` excluding node_modules = 79 |
...
| A short trimmed excerpt of changed lines is included | `translation-context.ts:215-219` `trimPatch` - redacts the full patch, then `.slice(0, 400)`; `lib/ai/evidence-resolver.ts` `trimExcerpt` - `.slice(0, 300)` |
| The redactor covers PEM private-key blocks, GitHub tokens and PATs, Stripe `sk_live_`/`rk_live_`/`whsec_`, credentialed Postgres/MySQL/MariaDB/Mongo/Redis URLs, 32+ hex, emails, and 20+ screaming-snake constants | `lib/validators/sanitize.ts:1-10` - eight patterns; fixtures in `lib/validators/sanitize.test.ts` |
| The redactor does **not** cover JWTs or AWS secret access keys | `lib/validators/sanitize.ts:1-10` - no pattern matches either. AWS key _IDs_ match only incidentally, via the 20+ screaming-snake rule at `:9` |
| Diff excerpts are redacted before they are truncated | `translation-context.ts:217-218`; boundary test in `translation-context.test.ts` - secret across character 400, asserts a 400-character excerpt ending `[REDACTED]` |
| Body excerpts are still truncated before they are redacted | `translation-context.ts:74` (`createExcerpt`, 240 chars) then `:88`; `lib/github/sources.ts:238` then `:159-160`. Recorded as a caveat, not claimed on the page |
| A GitHub enrichment failure returns redacted text, not the client's own source | `translation-context.ts:47`, `:52`, `:90`; `app/api/translate-work/route.ts:449-451`; covered in `translation-context.test.ts` and `app/api/translate-work/route.test.ts` |
| Invoice sources are redacted before they are persisted | `app/api/invoices/route.ts:164-166`; covered in `app/api/invoices/route.test.ts` |
| Redaction is fail-closed on plans without the entitlement | `translate-work/route.ts:430-432`, `app/api/github/sources/route.ts:69-71`, `lib/services/clients.ts:501-505` - all `redactionControl ? requested : true`; `lib/defaults.ts:12` defaults it on |
...
Claims that entered this draft from Nick directly rather than from the repo, dated **2026-08-25**. The three issue-fix rows below started here as Nick's direct answers and have since been re-sourced to merged code in diffbill PR #159; their evidence column now cites that code rather than the conversation. The three rows after them - diffbill #160, #161 and #162 - never came from Nick at all; they were found while verifying #159 and are sourced to merged code only.
...
| Translate-work's error fallback redacts server-side instead of trusting the client's sources | diffbill **#160** - **verified fixed in diffbill PR #163 (merged 2026-08-25), confirmed against code 2026-08-26.** Resolution chosen: redact, not refuse. `app/api/translate-work/route.ts:449-451`, `lib/github/translation-context.ts:47` and `:52`, `app/api/invoices/route.ts:164-166`; the gate at `translate-work/route.ts:430-432` matches `github/sources/route.ts:69-71`. Not from Nick |
| Diff excerpts are redacted before truncation, not after | diffbill **#161** - **verified fixed in diffbill PR #163 (merged 2026-08-25), confirmed against code 2026-08-26.** `lib/github/translation-context.ts:215-219`, with a boundary regression test in `translation-context.test.ts`. **Body excerpts still truncate first** (`translation-context.ts:74` before `:88`; `lib/github/sources.ts:238` before `:159-160`) - outside #161's scope and carried as a caveat in section 3. Not from Nick |
| The redactor's pattern set reaches more credential shapes | diffbill **#162** - **partial** (PR #163, merged 2026-08-25, confirmed against code 2026-08-26). Landed: PEM private-key blocks, Stripe `sk_live_`/`rk_live_`/`whsec_`, credentialed database connection strings (`lib/validators/sanitize.ts:2`, `:5`, `:6`). **Did not land: JWTs and AWS secret access keys.** No AWS pattern exists; key IDs match only via the pre-existing screaming-snake rule at `:9`. Not from Nick |
...
**Second verified set (2026-08-26).** Verifying PR #159 turned up three residual issues - diffbill **#160**, **#161** and **#162** - which PR #163 ("fix(core): close source redaction gaps") closed on 2026-08-25 as `6fed8995`. #160 and #161 are **verified fixed against the merged code**; #162 is **partial**, and section 3 is written to the shapes that actually landed rather than the ones the issue suggested. These rows did not come from Nick's answers. One thing stays open and is not a copy problem: PR body excerpts are still truncated before they are redacted, in both `lib/github/translation-context.ts` and `lib/github/sources.ts` - the same failure #161 named, on a field #161 did not cover. Nothing on this page claims otherwise, and it wants its own diffbill issue. Citations are in the notes at the end of section 3.
