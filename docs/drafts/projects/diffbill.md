...
**7. Two video decisions for you.** The wiring is now fully pinned in section 5, so nothing is blocked - but two judgment calls are yours. First, `client-portal.mp4` is 73 MB and `multi-repo.mp4` is 29.5 MB; if either goes on nickneely.dev they want re-encoding, and the changelog clips ship with no poster frame set, so one has to be chosen. Second, `hero.mp4` / `hero.webm` are a finished 20-second Remotion render that is currently referenced by nothing on the live site. It is 1.26 MB, it loops seamlessly, and it is the single best-fitting asset for a project detail page. Do you want it on the frontdoor page even though diffbill itself is not using it right now? I would say yes.
