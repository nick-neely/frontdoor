...
{/* DRAFT FOR NICK'S EDIT. This file is the editable surface for the diffbill detail page: composed from docs/drafts/projects/diffbill.md, which stays in the repository as provenance and carries the facts register mapping every claim below to the merged code it was verified against. The prose here is pre-voice-pass. Pictures were captured 2026-08-25 from the locally running apps and are colocated in content/projects/diffbill/. The two previously held figures - 07-core-ai-translation-review.png and 10-core-stripe-finalization.png - are now in place; you cleared them on 2026-08-25/26 and neither names a real client, so no sign-off is outstanding. A structural pass has since re-shaped the presentation without rewriting a sentence: the stack and data-flow paragraphs were split at their own seams, the PR-selection picture moved up beside the filtering sentence it illustrates, the measurement and its provenance became a callout, and two verbatim code excerpts were added. The excerpts are byte-identical to diffbill at commit 6fed8995 - apps/core-app/lib/github/translation-context.ts lines 215-219 and apps/core-app/lib/ai/evidence-resolver.ts lines 88-100 - so re-copy them if that repository moves. Both were checked for secret-shaped content: they carry no keys, environment names, internal URLs, or client names. */}
