...
export function parseSvelteComponentFile(content) {
  ...
  const scriptMatch = /^([\s\S]*?)<script\b[^>]*>[\s\S]*?<\/script>/i.exec(
    text
  );
  const withoutScript = scriptMatch ? text.slice(scriptMatch[0].length) : text;
  const styleMatch = /<style\b[^>]*>[\s\S]*?<\/style\s*>/i.exec(withoutScript);
...
export function extractMatchingSourceCss(routeSource, originalMarkup) {
  ...
  const styleMatch = /<style\b[^>]*>([\s\S]*?)<\/style\s*>/i.exec(
    String(routeSource || "")
  );
  if (!styleMatch) {
    return empty;
...
function styleBlockText(sourceText) {
  const match = /<style\b[^>]*>([\s\S]*?)<\/style\s*>/i.exec(
    String(sourceText || "")
  );
