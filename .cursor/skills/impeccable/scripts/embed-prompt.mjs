#!/usr/bin/env node
// Modified by Neely Labs in 2026 to locate PNG IEND through chunk parsing
// instead of scanning compressed image bytes.
...
function findPngChunkOffset(b, targetType) {
  let off = 8;
  while (off + 12 <= b.length) {
    const len = b.readUInt32BE(off);
    const type = b.toString("ascii", off + 4, off + 8);
...
if (isPng) {
  ...
  const iend = findPngChunkOffset(buf, "IEND");
  if (iend < 8) {
