...
(function () {
  ...
  function buildConfigureRow() {
    const controlsLocked = pendingApplyInFlight;
    const row = el("div", {
      alignItems: "stretch",
      display: "flex",
      height: CONFIGURE_BAR_H,
  ...
  function parseSourceLoc(loc) {
    const match = /^(\d+)(?::(\d+))?/.exec(String(loc || ""));
    return {
  ...
  function parseManualEditRefSegment(segment) {
    const nthMatch = /:nth-of-type\((\d+)\)$/.exec(String(segment || ""));
  ...
  function reportVariantMountFailed(sessionId, variantNum, moduleUrl, error) {
    ...
    const variant = Math.max(parsed, 1);
  ...
  async function sendEvent(msg, opts) {
    msg.token = TOKEN;
    ...
    return await sessionCreationGate.then(doSend);
  }
  ...
  async function sendCheckpoint(reason) {
    if (!currentSessionId) {
      return null;
    }
    return await sendEvent(checkpointPayload(reason)).catch(() => null);
  }

  async function sendSteerCheckpoint(id, reason, extra) {
    if (!id) {
      return null;
    }
  ...
  async function collectFontCssText() {
    ...
    return await inlineFontUrls(chunks.join("\n"));
  }
      ...
      if (shouldUseAncestorCropShaderProxy(el)) {
        try {
          return await hideCaptureChromeForShaderProxy(
            async () => await captureElementFromRenderedAncestor(ms, el, opts)
          );
        } catch (error) {
          console.warn(
...
void main() {
  ...
  function normalizeCssColor(v) {
    ...
    const s = v.trim();
    const oklch = /oklch\([^)]+\)/i.exec(s);
    if (oklch) {
      return oklch[0];
    }
    const hex = /#[0-9a-fA-F]{3,8}\b/.exec(s);
    if (hex) {
      return hex[0];
    }
