...
(function () {
  ...
  function checkColors(opts) {
    ...
    if (classList) {
      ...
      const grayMatch = /\btext-(?:gray|slate|zinc|neutral|stone)-\d+\b/.exec(
        classStr
      );
      const colorBgMatch =
        /\bbg-(?:red|orange|amber|yellow|lime|green|emerald|teal|cyan|sky|blue|indigo|violet|purple|fuchsia|pink|rose)-\d+\b/.exec(
          classStr
        );
      ...
      const purpleText = /\btext-(?:purple|violet|indigo)-\d+\b/.exec(classStr);
      if (
        purpleText &&
  ...
  function cssLengthToPx(value) {
    const m = /^(-?[\d.]+)(px|rem|em)$/i.exec(String(value || "").trim());
    if (!m) {
      return null;
    }
  ...
  function scanCssTextForPseudoStripe(rawContent) {
    ...
    while ((m = ruleRe.exec(content)) !== null) {
      ...
      const colorToken =
        /(?:rgba?|hsla?|oklch|oklab|lab|lch|hwb)\([^)]*\)|#[0-9a-f]{3,8}\b/i.exec(
          bg
        );
  ...
  function collectMarqueeKeyframes(content) {
    ...
    while ((m = re.exec(content)) !== null) {
      ...
      while (i < content.length && depth > 0) {
        const ch = content.codePointAt(i);
        if (ch === 0x7b /* { */) {
          depth++;
        } else if (ch === 0x7d /* } */) {
          depth--;
  ...
  function scanCssTextForPulsingDot(content, markup = content) {
    ...
    while ((cm = classRe.exec(markup)) !== null) {
      const cls = cm[1] || cm[2] || "";
      const anim = /\banimate-(ping|pulse)\b/.exec(cls);
      if (!anim) {
        continue;
      }
  ...
  function scanHtmlForShapeAssembledIllustration(html) {
    ...
    while ((m = svgRe.exec(html)) !== null) {
      const block = m[0];
      const openTag = (/^<svg\b[^>]*>/i.exec(block) || [""])[0];
  ...
  function buildHtmlPatternCorpora(html) {
    ...
    while ((m = tagRe.exec(source)) !== null) {
      const tag = m[0];
      const sm = /\bstyle\s*=\s*("[^"]*"|'[^']*')/i.exec(tag);
      if (sm) {
        styleParts.push(`style=${sm[1]}`);
      }
      const cm = /\bclass\s*=\s*(?:"([^"]*)"|'([^']*)')/i.exec(tag);
      if (cm) {
    ...
    while (current && current.nodeType === 1) {
      ...
      if (hasGradientOrUrl) {
        if (current.tagName === "BODY" || current.tagName === "HTML") {
          return flatten({ a: 1, b: 255, g: 255, r: 255 });
        }
        return null;
      }
  ...
  function parseColorMix(str) {
    const m = /^color-mix\(/i.exec(String(str).trim());
    if (!m) {
      return null;
    }
  ...
  function parseAnyColor(s) {
    ...
    m =
      /oklch\(\s*([\d.]+)(%?)\s*[\s,]*\s*([\d.]+)\s*[\s,]+\s*([-\d.]+)(?:deg)?(?:\s*\/\s*([\d.]+)(%)?)?\s*\)/i.exec(
        str
      );
    if (m) {
    ...
    m =
      /hwb\(\s*(-?[\d.]+)(?:deg)?\s+([\d.]+)%\s+([\d.]+)%(?:\s*\/\s*([\d.]+)(%)?)?\s*\)/i.exec(
        str
      );
    if (m) {
      const rgb = hwbToRgb(
  ...
  `;
    ...
    async function waitForVisualPaint() {
      ...
      lazyVisualContrastObserver = new IntersectionObserver(
        (entries) => {
          for (const entry of entries) {
            ...
            waitForVisualPaint()
              .then(async () => await analyzeVisualContrastCandidate(candidate))
              .then((result) => {
