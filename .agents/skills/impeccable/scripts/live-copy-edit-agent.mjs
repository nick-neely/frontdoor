...
async function runCodex(
  ...
  args.push("-");
  return await runAgentProcess("codex", args, prompt, {
    cwd,
    env,
    logPath,
    timeoutMs,
...
async function runClaude(
  ...
  return await runAgentProcess("claude", args, "", {
    cwd,
    env,
    logPath,
...
export function extractRunnerErrorMessage(output, command) {
  ...
  const trailingMatch = /\{[\s\S]*\}\s*$/.exec(text);
  if (trailingMatch) {
...
function computeCommandAuthed(command) {
  ...
  const parsed =
    tryParseJson(stdout) ||
    tryParseJson(/\{[\s\S]*\}\s*$/.exec(stdout)?.[0] || "");
